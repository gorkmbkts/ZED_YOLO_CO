...
def AkilliZeka(weights, img_size, conf_thres=0.2, iou_thres=0.45):
    ...
    while not exit_signal:
        if run_signal:
            ...
            for result in model.track(img, show=False, stream=True, agnostic_nms=True,verbose=False, persist=True ,conf=0.25):
...
def main():
    ...
    Ai_thread = Thread(target=AkilliZeka, kwargs={'weights': 'best.pt', 'img_size':640, "conf_thres": 0.5})
    Ai_thread.start()
